...
func (o *output) Send(e *Event) error {
	...
	// write directly here:
	// TODO: use sync.pool to write in another routine ?..
	...
	destLogString := fmt.Sprintf("[%s] [%s] [%s] - [%s] - [%s] - %s\n", e.Timestamp, e.Fields[0][0], Levels[e.Level], "log/output.go", e.Key, e.Message)
	_, err = o.f.Write( []byte(destLogString) )
	return err
