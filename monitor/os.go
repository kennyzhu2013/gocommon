...
var (
	...
	MaxCalls   = 400000 // to modify as 4000.
	UpLimits   = 360000
	DownLimits = 280000
)
...
func SetMaxCalls(iMax int) {
	MaxCalls = iMax
	UpLimits = int(float32(MaxCalls) * 0.9)
	DownLimits = int(float32(MaxCalls) * 0.7)
	if DownLimits == UpLimits {
		DownLimits = UpLimits - 1
	}
}
...
func UpdateServiceWithoutSync(service *registry.Service, weights int) {
	...
	for _, node := range service.Nodes {
		...
		if weights < DownLimits {
			...
			log.Warnf("UpdateServiceWithoutSync swtich to UpThresh status, session size: %v", weights)
			status = UpThresh
		} else {
			// nothing to do...
		}
		...
		// no change. return.
		if node.Metadata[ServiceStatus] == status {
			continue
		}

		node.Metadata[ServiceStatus] = status
		log.Warnf("UpdateServiceWithoutSync swtich to status:%v, session size: %v", status, weights)
	}
