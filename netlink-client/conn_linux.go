...
func (c *conn) Receive() ([]Message, error) {
	...
	// check message length , adn alloc message buf.
	for {
		// Peek at the buffer to see how many bytes are available.
		//
		// TODO(mdlayher): deal with OOB message data if available, such as
		// when PacketInfo ConnOption is true. change no wait with unix.MSG_PEEK | MSG_DONTWAIT.
		n, _, _, _, err := c.s.Recvmsg(b, nil, unix.MSG_PEEK )
		if err != nil {
			return nil, os.NewSyscallError("recvmsg", err)
		}

		// fmt.Printf("Recvmsg return length:%d", n)
		if n <= 0 {
			return nil, errors.New("no message receive ! ")
		}
...
// sock_fd = socket(AF_NETLINK, SOCK_RAW, USER_MSG); 功能..
