...
func (e *etcdv3Registry) Register(s *Service, opts ...RegisterOption) error {
	...
	if ok {
		if _, err := e.client.KeepAliveOnce(context.TODO(), leaseID); err != nil {
			if !strings.EqualFold(err.Error(), rpctypes.ErrorDesc(rpctypes.ErrGRPCLeaseNotFound)) {
				return err
			}
	...
	// create hash of service; uint64, here may be crashed.
	simpleS,_ := json.Marshal(s)
	h, err := hash.Hash(string(simpleS), nil)
	if err != nil {
		return err
	}
