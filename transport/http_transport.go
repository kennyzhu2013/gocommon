...
func (h *httpTransportClient) Send(m *Message) error {
	...
	//req, err := http.NewRequest("POST", m.Header["Micro-Endpoint"], b)
	//if err != nil {
	//	return err
	//}
	//req.Header = header
	//req.Host = h.addr
	path := m.Header["Micro-Endpoint"]
	...
	req := &http.Request{
		...
		URL: &url.URL{
			Scheme: "http",
			Host:   h.addr,
			Path:   path, // add path for http post.
		},
		Header:        header,
		Body:          b,
	...
	fmt.Println("httpTransportClient send:" + req.URL.Path)
