...
func FromContext(ctx context.Context) (traceID string, parentSpanID string, isFound bool) {
	traceID, traceOk := service_wrapper.Get(ctx, traceIDKey)
	microID, microOk := service_wrapper.Get(ctx, "Micro-Id")
	if !traceOk && !microOk {
		isFound = false
		return
	}
	...
	parentSpanID, ok := service_wrapper.Get(ctx, spanIDKey)
...
func ToContext(ctx context.Context, traceID, parentSpanID string) context.Context {
	return service_wrapper.MergeContext(ctx, map[string]string{
		traceIDKey: traceID,
